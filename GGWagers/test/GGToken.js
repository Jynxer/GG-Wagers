...
contract('GGToken', function(accounts) {
	...
	it('initialises the contract with the correct values', function() {
		return GGToken.deployed().then(function(instance) {
			...
			assert.equal(name, 'GG Token', 'has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol) {
			assert.equal(symbol, 'GG', 'has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard) {
			assert.equal(standard, 'GG Token v1.0', 'has the correct standard');
		});
	});
	...
	it('allocates the initial supply upon deployment', function() {
		return GGToken.deployed().then(function(instance) {
			...
			assert.equal(totalSupply.toNumber(), 1000000000, 'sets the total supply to 100,000,000')
	...
	it('transfers token ownership', function() {
		return GGToken.deployed().then(function(instance) {
			...
			assert.include(error.message.toString(), 'revert', 'error message must contain revert');
