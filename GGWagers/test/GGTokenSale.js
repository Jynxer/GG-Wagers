...
var GGTokenSale = artifacts.require("GGTokenSale");

contract('GGTokenSale', function(accounts) {
	...
	it('initialises the contract with the correct values', function() {
		return GGTokenSale.deployed().then(function(instance) {
	...
	it('facilitates token buying', function() {
		return GGToken.deployed().then(function(instance) {
			...
			assert.equal(receipt.logs[0].args._amount, numberOfTokens, 'logs the number of tokens purhcased');
			...
			assert(error.message.toString().indexOf('revert') >= 0, 'cannot purhcase more tokens than available');
		});
	});
	...
	it('ends token sale', function() {
		return GGToken.deployed().then(function(instance) {
			...
			assert.equal(balance.toNumber(), 999999990, 'returns all unsold GGTokens to admin');
			// Check that the contract has no balance
			return tokenInstance.balanceOf(tokenSaleInstance.address);
      	}).then(function(balance) {
      		assert.equal(balance.toNumber(), 0, 'checks that all tokens were removed from the contract');
		});
	});
